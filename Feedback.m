...
% pesq2(clean,sn',fs)
% pesq2(clean,Output_Total',fs)
% 

clean=audioread('filtered_clean(1).wav');
[sn,fs] =   audioread('gtr-dist-yes.wav');
sn      =   sn(:)';
sn      =   sn - mean(sn);
len    =   length(sn);
VAR_sn  =   var(sn);
...
% Parameters
% Gain_dB     = 20;           gain_Gz     = 10.^(Gain_dB./20);
gain_Gz     = 10;          
% Gain_dB     = 20 * log10 (GAIN_GZ);
...
v_W2z       = zeros(M2+DELAY,1);     % vector for probe input to W2z
...
Output_Total    = signal_audio_clipping(yn_total,gain_Gz);

subplot(3,1,1)
plot(clean)
title('clean signal')
%Output_Total=Output_Total/20;
...
subplot(3,1,3)
plot(Output_Total/10)
title('Output signal')


% a1=sum(clean.^2);
% b1=sum((Output_Total-clean).^2);
% b=sum((denoised_speech-clear_speech).^2);
% a2=sum(clean.^2);
% b2=sum((sn-clean).^2);
% 
% SNR_output=10*log10(a1/b1)
% SNR_feedback=10*log10(a2/b2)
PESQ_in=pesq2(clean,sn',fs)
PESQ_out=pesq2(clean,Output_Total',fs)
